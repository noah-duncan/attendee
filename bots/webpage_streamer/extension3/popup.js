...
document.getElementById('toggle').onclick = () => {
  if (recorder) {                     // ⇦ stop and save
    recorder.stop();
    return;
  }

  // ---- 1. Call tabCapture (callback style, works in all versions) ----
  chrome.tabCapture.capture({ audio: true, video: true }, stream => {
    if (chrome.runtime.lastError || !stream) {
      console.error('tabCapture failed:', chrome.runtime.lastError);
      document.getElementById('toggle').textContent = 'Error: ' + chrome.runtime.lastError.message;
      return;
    }

    captureScreenWithAudio(stream);

    return;

    // ---- 2. Keep the tab audible for the user (optional) ----
    ...
    recorder.onstop = () => {
      ...
      chrome.downloads.download({ url, filename: 'tab-audio.webm' });
      chunks = [];
